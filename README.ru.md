...
```bash
pytest -q                                                     # 20 смоук- и контрактных тестов
```
...
1. **Подключить к языковой модели.** MCP-сервер с предсказанием, таблицей скрининга и
   собственными ограничениями модели — и проверка, передают ли модели разного размера
   вердикт дальше или переступают через него.
