...
уверенность. Множитель, подобранный на валидации (×4.60), даёт 90 процентов
...
1H против 1T заметно выросли. Таблица целиком, включая одно число, которое ушло в
другую сторону, — в [карточке модели](MODEL_CARD.md).

Поставляемый ансамбль даёт **MAE 0.246 эВ (95% ДИ 0.226–0.268), RMSE 0.435,
...
|---|---|---|---|
| **Квазичастичная щель** — фотоэмиссия, транспорт | G₀W₀ из C2DB | 171 | **0.24 эВ** |
| **Энергия связи экситона** | Бете–Солпитер из C2DB | 171 | **0.13 эВ** |
| **Оптическая щель** = прямая G₀W₀ − экситон | по двум выше | 171 | **0.20 эВ** |
...
|---|---|---|
| Квазичастичная щель (G₀W₀) | 0.39 | **0.24** |
| Энергия связи (BSE) | 0.22 | **0.13** |
| Оптическая щель | 0.36 | **0.20** |
...
экстраполирует там, где полином может: без всех записей бор-азота h-BN выходит 4.82 эВ
против цели 5.74, тогда как полином даёт 5.56. Выше 5 эВ в подгонке всего четыре
материала. Из 1104 отложенных предсказаний это единственное место, где голова хуже, и
оно сообщается, а не прячется.
...
| | | **0.30 эВ** | 0.17 эВ в выборке, **0.71 эВ** leave-one-out | |

Старая выглядит лучше в этой таблице только потому, что эти пять строк — её обучающие
данные. На невиданном монослое она ошибается на 0.71 эВ, новая — на 0.30.
...
выводится. На четырёх TMD голова даёт **0.56 / 0.53 / 0.54 / 0.52 эВ** против BSE
...
одним примером, они выучивают одинаково и дружно соглашаются — ровно так фосфорен и
вышел уверенно неверным. Бэггинг должен чинить это в корне: треть членов любую
...
ловит случай, который раньше пропускал. Принят бэггинг всё равно не был, по трём
...
- **Калибровка действует только для той популяции, на которой подогнана.** Прогон
  ...
  Alexandria 2D тестовая MAE равна 0.246 эВ, на метастабильных структурах 0.509 эВ.
  Перекалибруйте
...
одинаковых данных. Он был измерен и **не принят**; числа ниже, в разделе
«Гипотеза, которая оказалась верной и всё равно проиграла».
