...
| **Alexandria 2D, ehull ≤ 0.1** | **13 349** | — | **0.261** | **без общих составов** |
...
монослоёв (лежат в [`examples/`](examples/), поэтому работает офлайн). PBE занижает
в среднем на +0.42 эВ; линейная поправка `exp ≈ 1.39·gap − 0.44` снижает
расхождение с 0.42 до 0.16 эВ. Поправка подгоняется только по точкам, которые
инструмент сам считает пригодными: предсказание, помеченное как вне области
применимости, не должно определять калибровку, которой правят все остальные.
...
1. Выложить загрузчик (`app.py`) на Hugging Face Spaces и связать его с обозревателем.
2. Конструктор семейств: прототип плюс два элемента, на выходе тепловая карта.
   Постоянные решётки брать из базы релаксированных структур, не из радиусов.
